...
def extract_and_decode(
    ...
    for param in params_list:
        name = param[0]
        sbyte = param[1] + 1  # offset by 1 for 1-based index
        shift = param[2]
        fmt_raw = param[3]
        ...
        try:
            ...
            # TAI_SECONDS
            if name == "TAI_SECONDS":
                results[name] = convert_tai_to_datetime(value*conversion_factor)

            if status_str is not None:
                if value < len(status_str):
                    results[name] = status_str[value]
                else:
                    results[name] = "---"
