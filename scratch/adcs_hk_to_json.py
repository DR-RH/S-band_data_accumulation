...
def convert_field_list_to_jsonable(field_list):
    result = []

    for name, start_byte, shift_bit, fmt, scale, status_strings in field_list:
        bit_width = FMT_TO_BIT_WIDTH[fmt]
...
jsonable_data = convert_field_list_to_jsonable(adcs_HK_list)

with open("adcs_HK_list.json", "w", encoding="utf-8") as f:
    json.dump(jsonable_data, f, ensure_ascii=False, indent=2)
