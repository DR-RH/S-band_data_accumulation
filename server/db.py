...
def ensure_payload_schema(conn: sqlite3.Connection, table_name: str) -> None:
    if table_name == MAIN_HK_TABLE:
...
def store_main_hk_payloads(db_path: Path, packet_id: str, df: pd.DataFrame, gse: str = "unknown") -> int:
    ...
    for row in df.to_dict("records"):
        data_hex = row["Data"]
        timestamp_obc_unix = _timestamp_unix(row["timestamp_obc"])
        rows.append(
            (
...
def store_adcs_hk_payloads(db_path: Path, packet_id: str, df: pd.DataFrame, gse: str = "unknown") -> int:
    ...
    for row in df.to_dict("records"):
        data_hex = row["Data"]
        timestamp_adcs_unix = _timestamp_unix(row["timestamp_adcs"])
        rows.append(
            (
